locals {
  ...
  tg3 = [aws_instance.app3_sub1.id, aws_instance.app3_sub2.id]
}
...
resource "aws_lb_target_group" "app_loadbalancer_tg3" {
  name     = "app3-tg"
  port     = 80
  protocol = "HTTP"
  vpc_id   = aws_vpc.main.id
...
resource "aws_alb_target_group_attachment" "tg3_loadbalancer_attach" {
  count            = length(local.tg3)
  target_group_arn = aws_lb_target_group.app_loadbalancer_tg3.arn
  target_id        = element(local.tg3, count.index)
  port = 8080
}
